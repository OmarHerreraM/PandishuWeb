...
exports.mpWebhook = functions.runWith({ timeoutSeconds: 30, memory: '128MB', labels: { environment: 'production', project: 'pandishu', owner: 'oscar', cost_center: 'sales' } }).https.onRequest(async (req, res) => {
    ...
    try {
        ...
        if (MP_WEBHOOK_SECRET_KEY && xSignature) {
            ...
            if (ts && v1) { const manifest = `id:${dataId};request-id:${xRequestId};ts:${ts};`; const expected = crypto.createHmac('sha256', MP_WEBHOOK_SECRET_KEY).update(manifest).digest('hex'); if (expected !== v1) { console.warn('?? MP Webhook: firma inv�lida'); return res.status(401).send('Unauthorized'); } }
        }
